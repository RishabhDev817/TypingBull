...
import { neonAudio } from './neonAudio';
...
export const NeonVelocityGame: React.FC<NeonVelocityGameProps> = ({ onBackToHub }) => {
  ...
  const [wordsDestroyed, setWordsDestroyed] = useState<number>(0);
  ...
  // Game Engine Mutable Refs (60+ FPS animation)
  const engineRef = useRef({
    words: [] as WordObstacle[],
    lasers: [] as LaserBeam[],
    particles: [] as Particle[],
    speedLines: [] as { x: number; y: number; length: number; speed: number; alpha: number }[],
    stars: [] as Star[],
    citySkyline: [] as SkylineBuilding[],
    lockedWordId: null as string | null,
    playerLaneX: 0.5,
  ...
  const triggerGameOver = useCallback((forceVictory = false) => {
    ...
    try {
      ...
      if (finalScore > savedHigh && finalScore > 0) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const accuracyCalc = engine.totalKeystrokes > 0
      ...
      if (elapsedMinutes > 0.04) {
        const currentWpm = Math.round((engine.correctKeystrokes / 5) / elapsedMinutes);
        setWpm(currentWpm);
    ...
    const loop = (currentTime: number) => {
      ...
      if (gameState === 'playing') {
        ...
        const spawnInterval = Math.max(
          MIN_SPAWN_INTERVAL_MS,
          BASE_SPAWN_INTERVAL_MS / speedScale
        );
        ...
        if (currentTime - engine.lastSpawnTime > spawnInterval) {
          ...
          const leastCrowdedLanes = [0, 1, 2, 3].sort((a, b) => laneWordCounts[a] - laneWordCounts[b]);
          ...
          engine.words.push({
            ...
            typedLength: 0,
            isTargeted: false,
            speed: Math.min(MAX_WORD_SPEED, BASE_WORD_SPEED * speedScale * (0.9 + Math.random() * 0.2)),
    ...
    >
      ...
      <div className="relative z-10 w-full px-6 py-4 flex items-center justify-between pointer-events-auto bg-gradient-to-b from-[#03020c]/95 via-[#03020c]/50 to-transparent backdrop-blur-[2px]">
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          {/* Enhanced Shield Progress Bar Container */}
          <div className="flex items-center gap-2.5 bg-slate-950/80 border border-cyan-500/40 px-3.5 py-2 rounded-2xl shadow-[0_0_15px_rgba(0,240,255,0.15)] backdrop-blur-md">
            ...
            <div className="flex flex-col">
              ...
              {/* Visible translucent grey empty track with thin glowing 1px border */}

...
export const WeakKeyPanel: React.FC<WeakKeyPanelProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`card-game card-halo-red p-5 pb-6 flex flex-col justify-between h-full min-h-[220px] overflow-visible ${className}`}
    >
      <div>
        {/* Header */}
        <div className="flex items-center gap-2 mb-3">
          <Target className="w-5 h-5 text-highlight-pink" />
          <h3 className="text-sm font-extrabold text-ink">🎯 Weak-Key Analysis</h3>
        </div>

        {/* Text-focused message */}
        <p className="text-xs text-body font-semibold leading-relaxed mb-3">
            ...
            {recommendation.weakKeys
              ...
              .map(({ key, errorRate }) => (
                <div key={key} className="flex items-center gap-2.5">
                  <span className="w-7 h-7 flex items-center justify-center text-xs font-extrabold rounded-xl text-white shrink-0"
                  ...
                  <span className="text-[10px] font-extrabold w-9 text-right shrink-0" style={{ color: errorRate > 0.3 ? '#E53935' : '#FF9800' }}>
      ...
      {recommendation.mission && onStartMission && (
        <motion.button
          ...
          className="w-full btn-chunky btn-chunky-orange text-xs py-2.5 cursor-pointer mt-2"
        >
